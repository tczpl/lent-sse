    ...
    def wanna_execute(self, address: int, annotation: DependencyAnnotation) -> bool:
        ...
        if address in self.storage_accessed_global:
            for location in self.sstores_on_path:
                try:
                    solver.get_model_wrapper((location == address,), caller = "wanna_execute1")
                    return True
        ...
        for location in storage_write_cache:
            for dependency in dependencies:
                ...
                try:
                    solver.get_model_wrapper((location == dependency,), caller = "wanna_execute2")
                    return True
            ...
            for dependency in annotation.storage_loaded:
                try:
                    solver.get_model_wrapper((location == dependency,), caller = "wanna_execute3")
                    return True
                except UnsatError:
                    continue
