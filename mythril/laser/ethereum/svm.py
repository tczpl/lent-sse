...
sym_info = {"sym_time": 0, "sym_cnt":0, "total_states":0, "create_time":0}
def get_sym_info():
    return sym_info
...
class LaserEVM:
    ...
    def sym_exec(
        ...
        if pre_configuration_mode:
            ...
            create_end_time = datetime.now()
            create_time = create_end_time-sym_start_time
            sym_info["create_time"] = create_time.total_seconds()
        ...
        for hook in self._stop_sym_exec_hooks:
            hook()
        sym_end_time = datetime.now()
        duration = sym_end_time-sym_start_time
        sym_info["sym_time"] += duration.total_seconds()
        sym_info["sym_cnt"] += 1
        sym_info["total_states"] =self.total_states
    ...
    def _execute_transactions(self, address):
        ...
        for i in range(self.transaction_count):
            ...
            print("self.use_reachability_check", self.use_reachability_check)
            if self.use_reachability_check:
                open_states_ = []
                for state in self.open_states:
                    print("checking use_reachability_check")
                    if state.constraints.is_possible(global_path=state.global_path, thistx_path=state.thistx_path):
                        open_states_.append(state)
                self.open_states = open_states_
            ...
            print("open_states", len(self.open_states))
            func_hashes = (
            ...
            execute_message_call(self, address, func_hashes=func_hashes, tx=i)
    ...
    #···································································
    def additionalSolverBeforExecution(self,global_state):
        if args.use_lent6!="1":
            return 
        
        instructions = global_state.environment.code.instruction_list
        op_code = instructions[global_state.mstate.pc]["opcode"]
        additionSolver=False

        if (op_code=="JUMP" or op_code=="JUMPI") and len(self.pre_hooks[op_code])>1 or len(self.post_hooks[op_code])>1 or (len(self.post_hooks[op_code])+len(self.pre_hooks[op_code]))>1:
            additionSolver=True
            
        if additionSolver==False:
            return 
            
        if global_state.world_state.is_skipped:
    
            if not global_state.world_state.is_final_checked:
                global_state.world_state.is_final_possible = global_state.world_state.constraints.is_possible()
                global_state.world_state.is_final_checked = True

            if not global_state.world_state.is_final_possible:
                if args.use_lent6 == "1":
                    args.impossible_paths_v1.add(global_state.world_state.thistx_path)
    #···································································

    def exec(self, create=False, track_gas=False, tx=0) -> Optional[List[GlobalState]]:
        """
        ...
        for global_state in self.work_list:
            global_state.world_state.thistx_path = "START"

        self.possible_paths_of_tx[tx] = set()
        ...
        for global_state in self.strategy:
            ...
            self.additionalSolverBeforExecution(global_state)

            try:                
                new_states, op_code = self.execute_state(global_state)
                # print("state.path",global_state.world_state.thistx_path)
            ...
            if len(new_states) > 1 and random.uniform(0, 1) < args.pruning_factor:
                ...
                for i in range(len(new_states)):
                    state = new_states[i]
                    possible_start_time = datetime.now()
                    append_path = "-"+str(global_state.mstate.pc)+":"+str(i)
                    state.world_state.global_path += append_path
                    state.world_state.thistx_path += append_path
                    # noprune
                    possible = True
                    print("checking", state.world_state.thistx_path, state.world_state.global_path)

                    if args.use_lent=="1":
                        # print("use lent")
                        # lent
                        if state.world_state.thistx_path in self.possible_paths:
                            print("lent skip", state.world_state.thistx_path)
                            possible = True
                        else:
                            print("lent check", state.world_state.thistx_path)
                            possible = state.world_state.constraints.is_possible(global_path=state.world_state.global_path, thistx_path=state.world_state.thistx_path)

                            if possible:
                                self.possible_paths.add(state.world_state.thistx_path)
                    
                    elif args.use_lent2=="1":
                        # print("use lent2")
                        # lent2
                        if not create:
                            if state.world_state.thistx_path in self.possible_paths:
                                print("lent2 skip", state.world_state.thistx_path)
                                possible = True
                            else:
                    ...
                    elif args.use_lent3=="1":
                        # print("use lent3")
                        # lent3
                        if tx>0 and state.world_state.thistx_path in self.possible_paths_of_tx[tx-1] and state.world_state.thistx_path in self.possible_paths_of_tx[tx]:
                                print("lent3 skip", state.world_state.thistx_path)
                                possible = True
                        else:
                            possible = state.world_state.constraints.is_possible()

                        if possible:
                            self.possible_paths_of_tx[tx].add(state.world_state.thistx_path)
                    
                    elif args.use_lent4=="1":
                        # print("use lent4")
                        # lent4
                        if global_state.world_state.is_skipped and global_state.world_state.is_final_checked and (not global_state.world_state.is_final_possible): # 父状态是否是错误跳过
                            print("lent4 unsatAsParent", state.world_state.thistx_path)
                            possible = False
                        else:
                            if state.world_state.thistx_path in self.possible_paths:
                                print("lent4 skip", state.world_state.thistx_path)
                                possible = True
                                state.world_state.is_skipped = True
                            else:
                                possible = state.world_state.constraints.is_possible()
                            if possible:
                                self.possible_paths.add(state.world_state.thistx_path)

                    elif args.use_lent5=="1":
                        # print("use lent5")
                        # lent5
                        if global_state.world_state.is_skipped and global_state.world_state.is_final_checked and (not global_state.world_state.is_final_possible): # 父状态是否是错误跳过
                            print("lent5 unsatAsParent", state.world_state.thistx_path)
                            possible = False
                        else:
                            if state.world_state.thistx_path in self.possible_paths and state.world_state.thistx_path not in args.impossible_paths: # 没出现过其他同错误跳过的
                                print("lent5 skip", state.world_state.thistx_path)
                                possible = True
                                state.world_state.is_skipped = True
                            else:
                                possible = state.world_state.constraints.is_possible()
                    ...
                    elif args.use_lent6=="1":
                        # print("use lent6")
                        # lent6
                        if global_state.world_state.is_skipped and global_state.world_state.is_final_checked and (not global_state.world_state.is_final_possible): # 父状态是否是错误跳过
                            print("lent6 unsatAsParent", state.world_state.thistx_path)
                            possible = False
                        else:
                            if state.world_state.thistx_path in self.possible_paths and state.world_state.thistx_path not in args.impossible_paths_v1: # 没出现过其他同错误跳过的
                                print("lent6 skip", state.world_state.thistx_path)
                                possible = True
                                state.world_state.is_skipped = True
                            else:
                                possible = state.world_state.constraints.is_possible()
                    ...
                    elif args.use_lent7=="1":
                        # print("use lent7")
                        # lent7
                        if state.world_state.thistx_path in self.possible_paths:
                            print("lent7 skip", state.world_state.thistx_path)
                            possible = True
                        else:
                            print("lent7 check", state.world_state.thistx_path)
                            possible_int = state.world_state.constraints.is_possible_wrapper(global_path=state.world_state.global_path, thistx_path=state.world_state.thistx_path)

                            if possible_int > 0:
                                self.possible_paths.add(state.world_state.thistx_path)
                                possible=True
                                
                            elif possible_int == 0: #UNKNOWN
                                possible=False
                                
                                print("lent7 unknown", state.world_state.thistx_path)
                                thistx_path_arr = state.world_state.thistx_path.split("-")
                                if len(thistx_path_arr) > 2:
                                    for i in range(len(thistx_path_arr)-1, 1, -1):
                                        temp = "-".join(thistx_path_arr[0:i])
                                        if temp in self.possible_paths:
                                            print("lent7 remove", temp)
                                            self.possible_paths.remove(temp)
                                            break
                            else:
                                possible=False


                    elif args.use_lent8=="1":
                        # print("use lent8")
                        # lent8
                        if state.world_state.thistx_path in self.possible_paths:
                            print("lent8 skip", state.world_state.thistx_path)
                            possible = True
                        else:
                            print("lent8 check", state.world_state.thistx_path)
                            possible_int = state.world_state.constraints.is_possible_wrapper(global_path=state.world_state.global_path, thistx_path=state.world_state.thistx_path)
                            ...
                            elif possible_int == 0: #UNKNOWN #TODO: Try <0
                                possible=False
                                
                                print("lent8 unknown", state.world_state.thistx_path)
                                thistx_path_arr = state.world_state.thistx_path.split("-")
                                if len(thistx_path_arr) > 2:
                                    for i in range(len(thistx_path_arr)-1, 1, -1):
                                        temp = "-".join(thistx_path_arr[0:i])
                                        if temp in self.possible_paths:
                                            print("lent8 remove", temp)
                                            self.possible_paths.remove(temp)
                            else:
                                possible=False
                    ...
                    elif args.use_lent9=="1":
                        # print("use lent9")
                        # lent9
                        if state.world_state.thistx_path in args.possible_paths and state.world_state.thistx_path not in args.recalled_near_paths:
                            print("lent9 skip", state.world_state.thistx_path)
                            possible = True
                        else:
                            print("lent9 check", state.world_state.thistx_path)
                            possible_int = state.world_state.constraints.is_possible_wrapper(global_path=state.world_state.global_path, thistx_path=state.world_state.thistx_path)

                            if possible_int > 0:
                                args.possible_paths.add(state.world_state.thistx_path)
                                if state.world_state.thistx_path in args.recalled_near_paths:
                                    args.recalled_near_paths.remove(state.world_state.thistx_path)
                                possible=True
                    ...
                    # sse1
                        ...
                        state.world_state.sse_depth += 1
                        global_state.childs.append(state)
                        state.parent=global_state
                        print("plus", global_state, global_state.world_state.sse_depth, state, state.world_state.sse_depth)
                        print("now?", global_state, global_state.parent)
                        if state.world_state.sse_depth < args.max_sse_depth:
                            print("sse ucc skip", state.world_state.thistx_path, state.world_state.sse_depth)
                            possible = True
                        else:
                            print("sse ucc check", state.world_state.thistx_path, state.world_state.sse_depth)
                            possible_int = state.world_state.constraints.is_possible_wrapper(global_path=state.world_state.global_path, thistx_path=state.world_state.thistx_path)
                            if possible_int > 0:
                                print("reachable, continue")
                                possible=True
                                state.world_state.sse_depth = 0
                            else:
                                possible=False
                                # Unsatisfiable Core Based Backtracking
                                print("uc backtrack", "get from global var")
                                z3_uc =  args.z3_uc
                                print("z3's uc", z3_uc)
                                temp_state = state
                                temp_arr = []
                                while True:
                                    temp_arr.append(temp_state)
                                    print("appending parent", len(temp_state.world_state.constraints))
                                    temp_state = temp_state.parent
                                    if temp_state == None or temp_state.world_state.sse_depth == 0:
                                        break
                                
                                left, right = 0, len(temp_arr) - 1

                                target_index = None
                                if z3_uc != None and len(z3_uc) > 0:
                                    # the max one is the k
                                    uc_k = 0
                                    for uc_str in z3_uc:
                                        if int(str(uc_str)) > uc_k:
                                            uc_k = int(str(uc_str))
                                    # larger than k will not be check
                                    for index in range(len(temp_arr)):
                                        target_index = index
                                        if len(temp_arr[index].world_state.constraints) <= uc_k:
                                            break
                                    
                                    # reversely in temp arr
                                    while target_index < len(temp_arr) -1:
                                        if temp_arr[target_index].world_state.constraints.is_possible_wrapper() > 0:
                                            # Got a reachable index.
                                            print(target_index, "possible")
                                            break
                                        else:
                                            # next one
                                            print(target_index, "impossible")
                                            target_index += 1

                                    # Previous one is unreachable
                                    if target_index>0:
                                        target_index -= 1
                                    ...
                                    print("Cannot get UC")
                                    while left <= right:
                                        print("searching", left, right)
                                        mid = left + (right - left) // 2
                                        if mid == left:
                                            target_index = mid
                                            break
                                        if temp_arr[mid].world_state.constraints.is_possible_wrapper() > 0:
                                            print(mid, "possible", temp_arr[mid])
                                            right = mid
                                            continue
                                        else:
                                            print(mid, "impossible", temp_arr[mid])
                                            left = mid
                                            continue
                                ...
                                if target_index != None:
                                    for temp_state in temp_arr[:target_index+1]:
                                        print("?", temp_state, len(temp_state.childs))
                                        for child in temp_state.childs:
                                            if child in self.work_list:
                                                self.work_list.remove(child)
                                                print("remove", child, "of", temp_state, len(self.work_list))              
                    ...
                    # sse2
                        ...
                        print("now?", global_state, global_state.parent)                        
                        # Ab
                        if (i==1 and len(possibles)>0 and possibles[0]==False) or state.world_state.sse_depth < args.max_sse_depth:
                            print("xxe ucc skip", state.world_state.thistx_path, state.world_state.sse_depth)
                            possible = True
                        else:
                            print("xxe ucc check", state.world_state.thistx_path, state.world_state.sse_depth)
                            possible_int = state.world_state.constraints.is_possible_wrapper(global_path=state.world_state.global_path, thistx_path=state.world_state.thistx_path)
                            if possible_int > 0:
                                print("reachable, continue")
                                possible=True
                    ...
                    possible_end_time = datetime.now()
                    duration = possible_end_time-possible_start_time
                    # print("is_possible_time", duration.total_seconds(), possible, state.world_state.global_path, state.world_state.thistx_path)
