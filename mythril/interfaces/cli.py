...
from mythril.support.model import get_total_info
from mythril.laser.ethereum.svm import get_sym_info
...
def get_runtime_input_parser() -> ArgumentParser:
    ...
    parser.add_argument("--use-lent")
    parser.add_argument("--use-lent2")
    parser.add_argument("--use-lent3")
    parser.add_argument("--use-lent4")
    parser.add_argument("--use-lent5")
    parser.add_argument("--use-lent6")
    parser.add_argument("--use-lent7")
    parser.add_argument("--use-lent8")
    parser.add_argument("--use-lent9")
    parser.add_argument("--use-sse")
    parser.add_argument("--use-xxe")
    parser.add_argument("--use-uce")
    parser.add_argument("--use-ucc")
    parser.add_argument("--use-ucx")
    parser.add_argument("--max-sse-depth")
    parser.add_argument(
        "-a",
        "--address",
        ...
        if args.graph:
            ...
            try:
                ...
                if args.output_json!=None:
                    ...
                    duration = analyze_end_time-analyze_start_time
                    report_dict = report.as_dict()
                    report_dict["time"] = duration.total_seconds()
                    report_dict["sym_time"] = get_sym_info()["sym_time"]
                    report_dict["sym_cnt"] = get_sym_info()["sym_cnt"]
                    report_dict["total_states"] = get_sym_info()["total_states"]
                    report_dict["create_time"] = get_sym_info()["create_time"]
                    report_dict["total_info"] = get_total_info()
                    with open(output_json, "w") as f:
                        f.write(json.dumps(report_dict, sort_keys=True))
                    
                # print(outputs[args.outform])
                # print("???")
                print("get_sym_info", get_sym_info())
                # print("get_total_info", get_total_info())
